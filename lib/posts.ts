    ...
    articlesOrSubCategories.forEach((item) => {
      ...
      if (fs.statSync(itemPath).isDirectory()) {
        ...
        if (fs.existsSync(mainMdPath)) {
          ...
          const author =
            (matterResult.data.author as string) || "Unknown Author";
          ...
          articlesInSubCategory.forEach((articleTitle) => {
            ...
            if (fs.existsSync(fullPath)) {
              ...
              const title =
                (matterResult.data.title as string) || "Untitled Post";
              const author =
                (matterResult.data.author as string) || "Unknown Author";
...
export async function getPostData(slug: string) {
  ...
  if (!matterResult.content || matterResult.content.trim() === "") {
    ...
    contentHtml = contentHtml.replace(
      /src="images\//g,
      `src="/assets/${category}/${articleTitle}/images/`
    );
    // Adjust sound paths to point to the public/assets directory
    contentHtml = contentHtml.replace(
      /src="sounds\//g,
      `src="/assets/${category}/${articleTitle}/sounds/`
    );
  }
